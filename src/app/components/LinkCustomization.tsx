...
const LinkCustomization = ({ links, removeLink, addNewLink }) => {
  return (
    <div className="flex-grow bg-white rounded-xl p-6 space-y-6">
      ...
      {/* Displaying Links or Placeholder */}
      {links.length === 0 ? ( // Conditional rendering
        <Placeholder />
      ) : (
          ...
          {links.map((link) => (
            <div key={link.id} className="h-[228px] p-5 bg-neutral-50 rounded-xl border flex-col justify-center items-center gap-3 inline-flex">
              <div className="self-stretch justify-between items-start inline-flex">
                ...
                <button
                  onClick={() => removeLink(link.id)} // Call removeLink with the link's id
      ...
      <footer className="border-t border-gray-300 p-6">
        <button className="w-full py-2 bg-[#623bff] text-white font-semibold rounded-lg opacity-50">
          Save
        </button>
      </footer>
    </div>
